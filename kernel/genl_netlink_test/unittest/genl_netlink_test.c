...
#define LOGC(f,arg...) printf("test:"f"\n",##arg);
...
static int genlmsg_send(int sockfd, unsigned short nlmsg_type, unsigned int nlmsg_pid,
        unsigned char genl_cmd,unsigned short nla_type, const void *nla_data, unsigned int nla_len)
{
...
static int genlmsg_get_family_id(int sockfd, const char *family_name)
{
    ...
    ret = genlmsg_send(sockfd, GENL_ID_CTRL, 0, CTRL_CMD_GETFAMILY,
...
static int test_netlink_unicast(void)
{
    ...
    ret = genlmsg_send(sockfd, id, pid, DOC_EXMPL_C_ECHO,
